double precision function qptpath (v, p) bind(c)
  use lib_array
  use Path
  implicit none
  ...
  double precision :: pln, prefac, db2, pwrv
  double precision :: tfil=0

  pln=4.0*2.30258509299
  db2=p(2)*p(2)
  prefac=p(1)*pln/(3.14159265359*db2*p(3))
...
!  write(*,1002) 'T= ', x, ' i= ', idx, ' (', lastX, ',', lastY, ')', ' P= ',lastPower
  ...
  if(z .ge. 1.0)then
     ...
     qptpath=prefac*pwrv*exp(-2.773*(x2+y2)/db2)*(-3.0*z*z + 2.0*z + 1.0)
  endif
